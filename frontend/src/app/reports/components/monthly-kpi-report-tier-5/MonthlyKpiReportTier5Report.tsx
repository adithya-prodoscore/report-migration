...
import { useMonthlyKpiReportTier5 } from '@/hooks/useMonthlyKpiReportTier5'
...
function ReportInner({ domainId, startDate, endDate }: ReportProps) {
  ...
  const { data, isLoading, isError, error } = useMonthlyKpiReportTier5(
    effectiveDomain,
    effectiveStart,
    effectiveEnd,
  )
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-neutral-50">
        ...
        <p className="mt-4 text-sm text-neutral-500">Loading Monthly KPI Report Tier 5…</p>
      </div>
    )
  }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              ...
              <p className="text-sm text-neutral-500 mt-0.5">
                {data.COMPANY.name} &middot; {data.COMPANY.period}
              </p>
            </div>
            <div className="flex gap-3 text-xs text-neutral-500">
              ...
              <span className="px-2.5 py-1 rounded-full bg-neutral-100 font-medium">
                {data.COMPANY.totalEmployees} employees
              </span>
            </div>
          </div>
        </div>
...
export function MonthlyKpiReportTier5Report(props: ReportProps) {
  return (
