...
const TIER_BAR: Record<string, string> = {
  critical: 'before:bg-kpi-tier-critical',
  monitor:  'before:bg-kpi-tier-monitor',
  high:     'before:bg-kpi-tier-high',
}
const TIER_SCORE_TEXT: Record<string, string> = {
  critical: 'text-kpi-tier-critical',
  monitor:  'text-kpi-gray-800',
  high:     'text-kpi-tier-high',
}

const STATUS_DOT: Record<string, string> = {
  'needs-attention': 'bg-kpi-red-500',
  'inactive':        'bg-kpi-yellow-500',
  'most-engaged':    'bg-kpi-blue-500',
}
...
function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  return (
    <span className={`ml-1.5 inline-block text-[10px] ${active ? 'opacity-100' : 'opacity-0'}`}>
      {dir === 'asc' ? '↑' : '↓'}
    </span>
  )
}

const COLUMNS: { key: SortKey; label: string; sortable: boolean; align?: string }[] = [
  ...
  { key: 'status',     label: 'Trend',      sortable: false, align: 'center' },
]
...
export function EmployeeTable({
  ...
  function handleDownloadCsv() {
    ...
    a.download = 'monthly_kpi_report_tier1.csv'
    a.click()
    ...
    <div className="bg-kpi-white border border-kpi-gray-100 rounded-xl overflow-hidden shadow-shadow-xs">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-kpi-gray-100 gap-4">
        <div className="flex items-baseline gap-4 min-w-0">
          ...
          <span className="text-[13px] text-kpi-gray-500 font-medium">
            Showing <strong className="text-kpi-gray-900 font-bold">{employees.length}</strong> of {employees.length}
          </span>
        </div>
        <button
          type="button"
      ...
      >
        <table className="w-full border-collapse border-spacing-0 text-[14px]" style={{ borderCollapse: 'separate' }}>
          <thead>
            <tr>
              {COLUMNS.map(col => (
                <th
                  ...
                  scope="col"
                  className={[
                    'bg-kpi-white text-[13px] font-semibold text-kpi-gray-700 text-left',
                    'px-5 py-4 border-b border-r border-kpi-gray-200 last:border-r-0',
                    'whitespace-nowrap select-none',
                    'sticky top-0 z-[3]',
                    col.key === 'name' ? 'sticky left-0 z-[4] min-w-[240px]' : '',
                    col.sortable ? 'cursor-pointer hover:text-kpi-blue-600' : '',
                    sortKey === col.key ? 'text-kpi-gray-950' : '',
                  ].join(' ')}
          ...
          <tbody>
            {paged.length === 0 && (
              <tr>
                <td colSpan={COLUMNS.length} className="py-16 text-center text-[14px] text-kpi-gray-500">
            ...
            {paged.map(emp => {
              ...
              return (
                <tr
                  ...
                  tabIndex={0}
                  onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') onRowClick(emp) }}
                >
                  {/* Name column — sticky left + tier bar */}
                  <td
                    className={[
                      'sticky left-0 z-[2] bg-kpi-white group-hover:bg-kpi-gray-50',
                      'pl-7 pr-5 py-[22px]',
                      'border-r border-b border-kpi-gray-100',
                      'relative min-w-[240px]',
                      isScrolled ? 'shadow-[6px_0_8px_-2px_rgba(0,0,0,0.12)]' : '',
                    ].join(' ')}
                  >
                    {/* Tier bar */}
                    <span
                      className={`absolute left-0 top-0 bottom-0 w-1 ${
                        tier === 'critical' ? 'bg-kpi-tier-critical' :
                        tier === 'high'     ? 'bg-kpi-tier-high' :
                                              'bg-kpi-tier-monitor'
                      }`}
                      aria-hidden="true"
                    />
                    <div className="flex items-center gap-2.5">
                      {/* Status dot */}
                      {hasDot ? (
                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${STATUS_DOT[emp.status] ?? 'bg-kpi-gray-300'}`} aria-hidden="true" />
                      ) : (
                  ...
                  {/* Role Avg */}
                  <td className="px-5 py-[22px] border-r border-b border-kpi-gray-100 bg-kpi-white group-hover:bg-kpi-gray-50 whitespace-nowrap text-kpi-gray-600">
                    {emp.roleAvg}
                  </td>
                  {/* Delta */}
                  <td className="px-5 py-[22px] border-r border-b border-kpi-gray-100 bg-kpi-white group-hover:bg-kpi-gray-50 whitespace-nowrap">
                    <span className={`inline-flex items-center gap-1.5 font-semibold text-[14px] ${
                      delta > 0 ? 'text-kpi-tier-high' : delta < 0 ? 'text-kpi-tier-critical' : 'text-kpi-gray-500'
                    }`}>
                  ...
                  {/* Active Time */}
                  <td className="px-5 py-[22px] border-r border-b border-kpi-gray-100 bg-kpi-white group-hover:bg-kpi-gray-50 whitespace-nowrap text-kpi-gray-800 font-medium">
                    {emp.activeTime}
                  </td>
                  {/* Trend sparkline */}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-5 py-3 border-t border-kpi-gray-100 text-[13px] flex-wrap gap-3">
          <span className="text-kpi-gray-600">
            Showing <strong className="text-kpi-gray-900 font-bold">{start + 1}–{Math.min(start + pageSize, employees.length)}</strong> of {employees.length}
          </span>
          <div className="flex items-center gap-1">
            <button
              className="w-8 h-8 flex items-center justify-center border border-kpi-gray-200 rounded-md text-kpi-gray-800 hover:bg-kpi-blue-50 hover:border-kpi-blue-400 hover:text-kpi-blue-700 disabled:text-kpi-gray-300 disabled:border-kpi-gray-100 disabled:cursor-not-allowed transition-colors"
              disabled={page === 1}
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1)
              ...
              .map((p, i) => p === '…'
                ? <span key={`ellipsis-${i}`} className="px-1 text-kpi-gray-400">…</span>
                : (
                  <button
                    key={p}
                    className={`w-8 h-8 flex items-center justify-center border rounded-md text-[13px] font-semibold transition-colors ${
                      page === p
                        ? 'bg-kpi-blue-600 text-white border-kpi-blue-600 hover:bg-kpi-blue-700'
                        : 'border-kpi-gray-200 text-kpi-gray-800 hover:bg-kpi-blue-50 hover:border-kpi-blue-400 hover:text-kpi-blue-700'
                    }`}
