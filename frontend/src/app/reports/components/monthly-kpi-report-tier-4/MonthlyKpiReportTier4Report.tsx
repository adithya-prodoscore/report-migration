...
import { useMonthlyKpiReportTier4 } from '@/hooks/useMonthlyKpiReportTier4'
...
function ReportInner({ domainId, startDate, endDate }: ReportProps) {
  ...
  const { data, isLoading, isError, error } = useMonthlyKpiReportTier4(
    effectiveDomain,
    effectiveStart,
    effectiveEnd,
  )
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-neutral-50">
        ...
        <p className="mt-4 text-sm text-neutral-500">Loading Monthly KPI Report Tier 4…</p>
      </div>
    )
  }
...
export function MonthlyKpiReportTier4Report(props: ReportProps) {
  return (
