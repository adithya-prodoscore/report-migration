...
// ---------------------------------------------------------------------------
// TypeScript types — mirroring the backend MonthlyKpiReportTier4Response
// ---------------------------------------------------------------------------
...
// EmployeeRecord allows extra dynamic keys (pivot + tool columns)
...
export interface MonthlyKpiReportTier4Data {
  ...
  DOMAIN_AVGS: unknown[]
  DOMAIN_DAILY: Record<string, DailyScore[]>
}
...
async function fetchMonthlyKpiReportTier4(
  domainId: string,
  startDate: string,
  endDate: string,
): Promise<MonthlyKpiReportTier4Data> {
  const base = process.env.NEXT_PUBLIC_API_URL ?? ''
  const url = `${base}/api/v1/monthly-kpi-report-tier-4?domain_id=${domainId}&start_date=${startDate}&end_date=${endDate}`
...
export function useMonthlyKpiReportTier4(
  domainId: string,
  startDate: string,
  endDate: string,
) {
  return useQuery({
    queryKey: ['monthly-kpi-report-tier-4', domainId, startDate, endDate],
    queryFn: () => fetchMonthlyKpiReportTier4(domainId, startDate, endDate),
