...
export interface MonthlyKpiReportTier6Data {
...
async function fetchMonthlyKpiReportTier6(
  domainId: string,
  startDate: string,
  endDate: string,
): Promise<MonthlyKpiReportTier6Data> {
  const base = process.env.NEXT_PUBLIC_API_URL ?? ''
  const url = `${base}/api/v1/monthly-kpi-report-tier-6?domain_id=${domainId}&start_date=${startDate}&end_date=${endDate}`
...
export function useMonthlyKpiReportTier6(
  domainId: string,
  startDate: string,
  endDate: string,
) {
  return useQuery({
    queryKey: ['monthly-kpi-report-tier-6', domainId, startDate, endDate],
    queryFn: () => fetchMonthlyKpiReportTier6(domainId, startDate, endDate),
